...
class DataValidation:
    ...
    def drop_misssing_values_columns(self,df:pd.Dataframe,report_key_name:str)->Optional[pd.DataFrame]:
        """
        this function will drop columns which contain misssing values more than specified threshold

        df: Accepts a pandas dataframe
        threhold:percentage criteria to drop a column
        ====================================================================
        returns Pandas Dataframe if altleast a single column is available after missing column drop else none
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            base_df = self.data_validation_config.base_file_path
            base_df.replace({"na":np.NAN},inplace=True)

            logging.info(f"Replace na value in base df")
            #base_Df has na as null
            logging.info(f"Drop null values colums from base df")
            base_df = self.drop_misssing_values_columns(df=base_df,report_key_name="missing_values_within_base_dataset")
            ...
            train_df = self.drop_misssing_values_columns(df=train_df,report_key_name="missing_values_within_train_dataset")
            logging.info(f"Drop null values colums from test df")
            test_df = self.drop_misssing_values_columns(df=test_df,report_key_name="missing_values_within_test_dataset")
            ...
            train_df_columns_status = self.is_required_columns_exist(base_df=base_df, current_df = train_df,report_key_name="missing_within_train_dataset")
            
            logging.info(f"Is all required columns present in test df")
            test_df_columns_status = self.is_required_columns_exist(base_df=base_df, current_df = test_df,report_key_name="missing_within_test_dataset")
